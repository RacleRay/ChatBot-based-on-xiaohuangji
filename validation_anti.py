...
def test(params):
    ...
    save_path = 'model_anti_best/best_cost.ckpt' # your path and file
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            pred, beam_prob = model_pred.predict(
                sess,
                np.array(x),
                np.array(xl)
            )
            # print('预测结果ids: ', pred[0])
            # print('预测分数: ', beam_prob[0])

            # 转换为words
            print(ws.inverse_transform(x[0]))
            # for i, s in enumerate(pred):
            #     ans = ws.inverse_transform(s)
            #     print('预测结果words: ',i , ans)
            if len(pred[0]) - pred[0].tolist().count(3) <= 4:
                if len(pred[3]) - pred[3].tolist().count(3) > \
                   len(pred[4]) - pred[4].tolist().count(3) and\
                   len(pred[3]) - pred[3].tolist().count(3) < 24:
                    ans = ws.inverse_transform(pred[3])
                else:
                    ans = ws.inverse_transform(pred[4])
            else:
